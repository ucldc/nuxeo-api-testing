...
def main(params):
    ...
    nuxeoapi_uids = [record['uid'] for record in nuxeoapi_parent_data]
    nuxeoapi_uids.sort()
    dbquery_uids = [record['uid'] for record in dbquery_parent_data]
    dbquery_uids.sort()

    # with open('nuxeoapi_uids.json', 'w') as f:
    #     f.write(json.dumps(nuxeoapi_uids))

    # with open('dbquery_uids.json', 'w') as f:
    #     f.write(json.dumps(dbquery_uids))

    print(f"parent uids equal?: {nuxeoapi_uids == dbquery_uids}")
    ...
    nuxeoapi_child_uids = [record['uid'] for record in nuxeoapi_child_data]
    nuxeoapi_child_uids.sort()
    dbquery_child_uids = [record['uid'] for record in dbquery_child_data]
    dbquery_child_uids.sort()

    print(f"child uids equal?: {nuxeoapi_child_uids == dbquery_child_uids}")
