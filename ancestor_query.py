...
def get_pages_of_documents(uid):
    documents = []
    page_index = 0
    next_page_available = True
    while next_page_available:
        response = get_ancestors(uid, page_index=page_index)
...
def get_ancestors(uid, page_index):
    query = (
        "SELECT * FROM SampleCustomPicture, CustomFile, CustomVideo, CustomAudio, CustomThreeD "
        f"WHERE ecm:ancestorId = '{uid}' AND "
        "ecm:isVersion = 0 AND "
        "ecm:isTrashed = 0 "
        "ORDER BY ecm:path, ecm:pos, ecm:uuid"
    )
...
path = '26fa054a-f967-49e7-a773-36694f671a1d' # /asset-library/UCR/SCUA/Archival/Klein/Publish/2017_pilot
...
with open('output/klein-ancestorquery.json', 'w') as f:
